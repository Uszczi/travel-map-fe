...
export default function DistancePicker({
  ...
  const t = useTranslations('routeOptions.distance');
    ...
    >
      <div className="space-y-2">
        <div className="flex items-end justify-between">
          <div className="space-y-0.5">
            ...
            <div className="text-xs text-zinc-500">{t('rangeHint', { min, max })}</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          aria-label={t('decrementAriaLabel', { step })}
        >
        ...
        <div className="flex-1 flex items-center gap-3">
          <input
            ...
            aria-valuetext={t('sliderValueText', { current })}
          ...
          <div className="relative w-36">
            ...
            />
            <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-zinc-500 select-none">
              {t('unit')}
            </span>
          </div>
        </div>
        ...
        <button
          ...
          onClick={inc}
          disabled={value >= max}
          title={t('incrementTitle', { step })}
          className={[
            'relative inline-flex items-center justify-center px-3 py-2.5  border',
            'transition-transform duration-100 hover:border-zinc-700 active:translate-y-px',
            value >= max ? 'opacity-60 cursor-not-allowed' : '',
          ].join(' ')}
          aria-label={t('incrementAriaLabel', { step })}
        >
